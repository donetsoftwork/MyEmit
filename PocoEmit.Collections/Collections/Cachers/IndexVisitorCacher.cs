...
internal class IndexVisitorCacher(ICacher<Type, IElementIndexVisitor> cacher)
    : CacheBase<Type, IElementIndexVisitor>(cacher)
{
    /// <inheritdoc />
    ...
    /// <returns></returns>
    private static DictionaryIndexVisitor CreateByDictionary(Type dictionaryType)
    {
        var arguments = ReflectionHelper.GetGenericArguments(dictionaryType);
        if (arguments.Length != 2)
            return null;
        return new DictionaryIndexVisitor(dictionaryType, arguments[0], arguments[1]);
    }
}
