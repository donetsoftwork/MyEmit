...
namespace PocoEmit.CollectionsUnitTests.Converters.ToStack;
...
public class FromArrayTests : CollectionTestBase
{
    ...
    public void Convert_User2DTO()
    {
        ...
        var result = _mapper.Convert<User[], Stack<UserDTO>>(source);
    ...
    public void Convert_UserArray2DTO()
    {
        ...
        var result = _mapper.Convert<UserArray, UserDTOStack>(source);
        Assert.NotNull(result);
        Assert.Equal(source.Name, result.Name);
        var resultItems = result.Users;
        Assert.Equal(sourceItems.Length, resultItems.Count);
        Equal(sourceItems[0], resultItems.Last());
    }
    [Fact]
    public void ConfigureMap()
    {
        ...
        var converter = _mapper.GetConverter<AutoUserDTO[], Stack<User>>();
